...
function render(currentTime) {
    ...
    const sunAngle = now * 45 * DEG2RAD;
    const earthRevolution = now * 30 * DEG2RAD;
    const earthRotation = now * 180 * DEG2RAD;
    const moonRevolution = now * 360 * DEG2RAD;
    const moonRotation = now * 180 * DEG2RAD;

    axes.draw(mat4.create(), mat4.create());

    // Sun
    let sunMatrix = mat4.create();
    mat4.rotate(sunMatrix, sunMatrix, sunAngle, [0, 0, 1]);
    mat4.scale(sunMatrix, sunMatrix, [0.2, 0.2, 1.0]);
    drawObject(sunMatrix, [1.0, 0.0, 0.0, 1.0]); // Red

    // Earth
    let earthMatrix = mat4.create();
    mat4.rotate(earthMatrix, earthMatrix, earthRevolution, [0, 0, 1]);
    ...
    mat4.rotate(moonMatrix, moonMatrix, earthRevolution, [0, 0, 1]); // 공전 원점은 Sun 기준
    mat4.translate(moonMatrix, moonMatrix, [0.7, 0.0, 0.0]); // Earth 위치
    mat4.rotate(moonMatrix, moonMatrix, moonRevolution, [0, 0, 1]); // Moon 공전
    mat4.translate(moonMatrix, moonMatrix, [0.2, 0.0, 0.0]); // Earth 기준 공전
    mat4.rotate(moonMatrix, moonMatrix, moonRotation, [0, 0, 1]); // Moon 자전
