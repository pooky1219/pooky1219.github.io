...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            shadingLevels = parseInt(event.key);
            updateText(textOverlay3, "toon levels: " + shadingLevels);
            shader.use();
            shader.setInt("u_levels", shadingLevels);
            render();  // 즉시 렌더링 업데이트
        }
    });
}
...
async function main() {
    try {
        ...
        shader.setVec3("material.diffuse", [0.0, 0.5, 0.5]);
        ...
        textOverlay3 = setupText(canvas, "toon levels: " + shadingLevels, 3);
