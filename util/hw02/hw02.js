...
function setupKeyboardEvents() {
    window.addEventListener('keydown', (event) => {
        let [x, y] = squarePos;
        if (event.key === 'ArrowUp' && y + 0.1< 1) y += moveStep;
        if (event.key === 'ArrowDown' && y - 0.1> -1) y -= moveStep;
        if (event.key === 'ArrowLeft' && x - 0.1 > -1) x -= moveStep;
        if (event.key === 'ArrowRight' && x + 0.1 < 1) x += moveStep;
        squarePos = [x, y];
    });
}
